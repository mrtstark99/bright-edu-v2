...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $db->prepare("
                UPDATE services
                SET title = ?, description = ?, content = ?, icon = ?, display_order = ?, status = ?, updated_at = datetime('now','localtime')
                WHERE id = ?
            ");

            if ($stmt->execute([$title, $description, $content, $icon, $display_order, $status, $id])) {
                redirect('/admin/services', 'Dịch vụ đã được cập nhật thành công.', 'success');
            } else {
                $errors[] = 'Có lỗi xảy ra khi cập nhật dịch vụ.';
            }
        } catch (Exception $e) {
...
?>
    ...
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-2xl font-bold text-midnight font-display">Chỉnh sửa dịch vụ</h1>
                ...
                <div class="bg-white rounded-3xl shadow-soft border border-slate-100 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div class="mb-6">
                            ...
                            <input type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors" id="title" name="title" value="<?php echo htmlspecialchars($_POST['title'] ?? $service['title']); ?>" required placeholder="Nhập tên dịch vụ...">
                        </div>

                        <div class="mb-6">
                            <label for="description" class="block text-sm font-bold text-slate-700 mb-2">Mô tả ngắn <span class="text-red-500">*</span></label>
                            <textarea class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors" id="description" name="description" rows="3" required placeholder="Mô tả ngắn gọn về dịch vụ..."><?php echo htmlspecialchars($_POST['description'] ?? $service['description']); ?></textarea>
                ...
                <div class="bg-white rounded-3xl shadow-soft border border-slate-100 overflow-hidden sticky top-6">
                    ...
                    <div class="p-6">
                        <div class="mb-6">
                            ...
                            <input type="text" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors" id="icon" name="icon" value="<?php echo htmlspecialchars($_POST['icon'] ?? $service['icon']); ?>" placeholder="Ví dụ: bi bi-star">
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors" id="display_order" name="display_order" value="<?php echo htmlspecialchars($_POST['display_order'] ?? $service['display_order']); ?>">
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <select class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500 transition-colors" id="status" name="status">
                                <option value="active" <?php echo ($_POST['status'] ?? $service['status']) === 'active' ? 'selected' : ''; ?>>Hoạt động</option>
                                <option value="inactive" <?php echo ($_POST['status'] ?? $service['status']) === 'inactive' ? 'selected' : ''; ?>>Ẩn</option>
                            </select>
                        </div>
